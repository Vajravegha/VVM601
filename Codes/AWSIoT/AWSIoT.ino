...
const char apn[]      = ""; //APN automatically detects for 4G SIM in most cases, else set APN provided by your Network Operator
...
void setup()
{
  ...
  mqtt.setServer(MQTT_HOST, 8883);  //broker domain name and port number, 8883 is for Secure MQTT
...
void loop()
{
  ...
  if (mqtt.connected()) 
  {
    if(mqtt.publish(TOPICNAME, "Hello...Secured Data Transmitted"))
    {
  ...
  delay(3000);  // Delay after publishing (adjust as needed)
}
